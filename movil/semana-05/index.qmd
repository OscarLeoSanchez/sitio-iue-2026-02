...
conectas pantallas entre sí. Empiezas con una app nueva y simple, MiniMarket, para entender
`Navigator.push` y `Navigator.pop` sin distracciones. Después construyes un proyecto de práctica más
completo, `taller_login`, con una pantalla de login y una pantalla de inicio, y profundizas con
rutas con nombre, paso de datos y navegación anidada.

Al terminar la guía sabes:

1. Navegar de una pantalla a otra con `Navigator.push` y entender la pila de navegación.
2. Pasar datos sencillos a otra pantalla mediante el constructor del widget.
3. Registrar rutas con nombre en `MaterialApp` y navegar con `Navigator.pushNamed`.
4. Pasar datos de una pantalla a otra con una clase de argumentos tipada.
5. Recibir de vuelta un resultado con `Navigator.pop` y `await Navigator.push`.
6. Construir una navegación anidada mínima con `BottomNavigationBar` e `IndexedStack`.

::: {.callout-note}
## Antes de empezar

Necesitas el entorno de Flutter funcionando desde la Semana 1, y ya conoces `Scaffold`, `Column`,
`Row`, `ListView`, `ListView.builder`, `TextField` y botones de la Semana 4: esta guía no los
reexplica, solo los usa.

Esta guía usa Flutter 3.41.9 (canal stable) y Dart SDK 3.11.5, las mismas versiones verificadas
desde la Semana 2.
:::

## Parte 1. Que es una pantalla

Flutter no tiene una entidad especial llamada "pantalla". Una pantalla es, sencillamente, un widget
que normalmente devuelve un `Scaffold`. Recuerda cómo arranca cualquier app que ya construiste:
...
`home` indica cuál widget se muestra al abrir la app. Una versión mínima de `ProductosScreen` es
esto:
...
Que una pantalla sea aproximadamente lo mismo que un widget importa porque, más adelante en esta
guía, `Navigator.push` en el fondo lo que hace es colocar otro widget encima del actual.

## Parte 2. El listado de MiniMarket

Vas a construir MiniMarket, una app de dos pantallas: un listado de productos y el detalle de uno
de ellos. Para el listado reutilizas exactamente lo que ya sabes de `ListView.builder` de la Semana
04, con una lista de mapas como datos:
...
## Parte 3. Una segunda pantalla, vacía

Cuando el usuario toca "Manzana", necesitas mostrar otra pantalla. Créala primero vacía, sin
conectarla todavía a nada:
...
Ahora mismo `ProductosScreen` y `DetalleProductoScreen` existen las dos, pero están desconectadas:
nada en la app sabe todavía cómo pasar de una a otra.

## Parte 4. Navigator.push

Flutter administra las pantallas visibles con un objeto llamado `Navigator`. Su operación
fundamental es `push`: agregar una pantalla nueva encima de la actual. Pruébalo primero con un
botón, sin mezclarlo todavía con la lista:
...
ElevatedButton(
  onPressed: () {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) {
          return const DetalleProductoScreen();
        },
      ),
    );
  },
...
|---|---|
| `Navigator` | El objeto que administra las pantallas visibles de la app |
| `push` | Agrega una pantalla nueva encima de la actual |
| `context` | Permite a Flutter ubicar este widget en el árbol y encontrar el `Navigator` correspondiente |
| `MaterialPageRoute` | Describe la ruta nueva: qué pantalla mostrar y con qué transición |
| `builder` | Recibe el `context` y devuelve el widget de la pantalla nueva |

El `builder` con `return` explícito se puede simplificar con una función flecha, porque el cuerpo es
una sola expresión:

```dart
builder: (context) => const DetalleProductoScreen(),
```

Vas a usar esta forma más corta de aquí en adelante.

## Parte 5. La pila de navegación

Antes del `push` solo `ProductosScreen` está visible. Después del `push`, `DetalleProductoScreen`
queda visible y `ProductosScreen` sigue debajo: no desapareció, por eso más adelante se puede volver
a ella.

::: {.diagrama}
![](imagenes/pila-navegacion-minimarket.svg){fig-alt="Pila de navegacion en MiniMarket: ProductosScreen queda en la base de la pila, Navigator.push apila DetalleProductoScreen encima, Navigator.pop la retira y vuelve a mostrar ProductosScreen"}

ProductosScreen no desaparece: sigue debajo, por eso Navigator.pop puede volver a ella.
:::

Un buen momento para notar algo que Flutter hace solo: como `DetalleProductoScreen` tiene un
`AppBar` con `title`, aparece automáticamente una flecha de regreso en esa barra, sin que la hayas
puesto tú. Aparece porque el `Navigator` sabe que hay una pantalla debajo en la pila.

## Parte 6. Navigator.pop

`Navigator.pop(context)` quita la pantalla actual de la pila y vuelve a mostrar la anterior. Frase
para recordar: `push` agrega una pantalla, `pop` quita la pantalla actual.

Agrega un botón "Volver" al final del `body` de `DetalleProductoScreen`:
...
## Parte 7. Navegar desde el ListView

Quita el botón "Ver detalle" de la Parte 4: ya no lo necesitas. En su lugar, agrega `onTap` al
`ListTile` del listado, con el mismo `Navigator.push` de antes:
...
Con esto, tocar cualquier producto del listado abre `DetalleProductoScreen`. Pero los tres
productos abren exactamente la misma pantalla, con el mismo texto fijo "Detalle del producto". Si
tocas Manzana o tocas Leche, ves lo mismo: la pantalla de detalle todavía no sabe cuál producto se
tocó.

## Parte 8. Pasar datos por el constructor

Necesitas que la pantalla de detalle reciba información sobre qué producto abrir. Todavía no hace
falta nada más complicado que lo que ya conoces de widgets con parámetros: el constructor.
...
::: {.diagrama}
![](imagenes/flujo-listview-detalle.svg){fig-alt="Flujo de navegacion desde una lista: el usuario toca un ListTile del ListView, se dispara onTap, onTap llama a Navigator.push con los datos del producto, y se muestra DetalleProductoScreen con ese nombre y precio"}

Cada producto de la lista abre el mismo widget, pero con datos distintos.
:::

Ahora cada producto abre su propio detalle: `nombre` y `precio` viajan de `ProductosScreen` a
`DetalleProductoScreen` por el constructor, exactamente igual que cualquier otro widget con
parámetros que ya construiste.

::: {.callout-tip}
## Reto 1: un producto nuevo

Agrega "Cafe $12.000" al listado, sin modificar `DetalleProductoScreen`. Confirma que la pantalla de
detalle funciona para el producto nuevo sin ningún cambio: eso demuestra que sirve para cualquier
producto, no solo para los tres originales.
:::

::: {.callout-tip}
## Reto 2: una descripcion

Agrega un campo `descripcion` a cada mapa de producto y muéstralo en `DetalleProductoScreen`. Vas a
necesitar agregar un parámetro `descripcion` al constructor y pasarlo desde el `onTap`, igual que
hiciste con `nombre` y `precio`.
:::

::: {.callout-tip}
## Reto 3: una tercera pantalla

Crea una pantalla `AcercaDeScreen` (un `Scaffold` con `AppBar` "Acerca de" y un texto en el `body`)
y navega hacia ella con `Navigator.push` desde un botón en `ProductosScreen`. El objetivo es que
apliques el patrón de la Parte 4 por tu cuenta, sin mirar el código de arriba.
:::

::: {.callout-note collapse="true"}
## Una solución posible del Reto 2
...
Con `Navigator.push`, la pila y el paso de datos por el constructor entendidos, ya tienes el patrón
básico y sólido. Ahora profundizas con un ejemplo más completo, `taller_login`, que introduce piezas
que en apps reales se vuelven necesarias: rutas con nombre en vez de pasar el widget directo, una
clase de argumentos para paso de datos más robusto, retorno de resultado y una barra de navegación
inferior real.

## Parte 9. La pantalla de login
...
## Parte 10. Pila de navegación en taller_login
...
## Parte 12. Paso de datos entre pantallas con una clase de argumentos
...
- [ ] MiniMarket navega del listado al detalle con `Navigator.push` y datos por el constructor
...
[Abrir la actividad](actividad-navegacion/index.html){.btn-doc target="_blank"}
:::

::: {.acceso-item}
Actividad de widgets de Flutter

[Abrir la actividad](actividad-widgets/index.qmd){.btn-doc target="_blank"}
:::
:::
